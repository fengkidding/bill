...
public class NioServerTest {
     ...
     * 接入事件处理器
     */
    private static void acceptHandler(ServerSocketChannel serverSocketChannel,
         ...
         * 回复客户端提示信息
         */
        socketChannel.write(Charset.forName("UTF-8").encode("连接成功，返给client数据"));
    }
    ...
    private static void readHandler(SelectionKey selectionKey, Selector selector) throws IOException {
        ...
        if (request.length() > 0) {
            ...
            // 广播给其他客户端
            NioServerTest.broadCast(selector, socketChannel, request);
        }
    }
        ...
        while (true) { // while(true) c for;;
            ...
            while (iterator.hasNext()) {
                ...
                if (selectionKey.isAcceptable()) {
                    NioServerTest.acceptHandler(serverSocketChannel, selector);
                }
                ...
                if (selectionKey.isReadable()) {
                    NioServerTest.readHandler(selectionKey, selector);
                }
            }
        }
    }
